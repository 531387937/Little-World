...
public class Text_AnimationCurve : MonoBehaviour {
...
//		this.transform.position = new Vector3 (-1.3f, anim.Evaluate (popUp) + 8.5f, 0.0f);
		...
		if (bee.Produced == true) {
			popUp += 0.1f;
//			this.transform.position = new Vector3 (-1.3f, anim.Evaluate (popUp) + 8, 0.0f);
		...
		if (popUp >= 20.0f) {
			popUp = 0.0f;
			this.transform.position = new Vector3 (-6.0f, -1.5f, 0.0f);
			bee.Produced = false;
		}
	}
